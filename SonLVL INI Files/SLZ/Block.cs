...
namespace S1ObjectDefinitions.SYZ
{
	class Block : ObjectDefinition
	{
		...
		{
			img = ObjectHelper.MapASMToBmp(ObjectHelper.OpenArtFile("../artnem/SLZ Platforms.nem", CompressionType.Nemesis), "../_maps/Floating Blocks and Doors.asm", 0, 2);
			for (int i = 0; i < 6; i++)
				imgs.Add(ObjectHelper.MapASMToBmp(ObjectHelper.OpenArtFile("../artnem/SLZ Platforms.nem", CompressionType.Nemesis), "../_maps/Floating Blocks and Doors.asm", i, 2));
		}
